...
STOCK_WORKSHEET = "股票追蹤"               # 熱門標的寫入的分頁（不存在會自動建立）
WORDS_WORKSHEET = "熱門詞彙"               # 高頻詞寫入的分頁（不存在會自動建立）
...
def _get_worksheet(spreadsheet, title: str, header: list[str]):
    """取得工作表；不存在就建立，空表則補上表頭列。"""
    import gspread
    try:
...
def write_to_google_sheets(stock_rows: list[list], word_rows: list[list]) -> None:
    """把「熱門標的」與「高頻詞」分別 Append 到試算表的兩個分頁。

    每天執行一次，資料會不斷往下累積，配合「檢查日期」欄位即可用
    篩選器／樞紐分析表觀察每檔股票熱度與詞彙的逐日變化。
    若找不到憑證檔則改用「乾跑模式」，只印出資料不實際寫入。
    """
    if not os.path.exists(CREDENTIALS_FILE):
        print(f"\n[乾跑模式] 找不到 {CREDENTIALS_FILE}，以下為「將要寫入」的資料：")
        print(f"\n--- 分頁「{STOCK_WORKSHEET}」 ---")
        print("  檢查日期     | 代碼  | 公司     | 提及次數 | 股價公式")
        for r in stock_rows:
            print(f"  {r[0]} | {r[1]}  | {r[2]:<6} | {r[3]:>6}   | {r[4]}")
        print(f"\n--- 分頁「{WORDS_WORKSHEET}」 ---")
        print("  檢查日期     | 排名 | 詞彙     | 次數")
        for r in word_rows:
            print(f"  {r[0]} | {r[1]:>3}  | {r[2]:<6} | {r[3]:>4}")
    ...
    # --- 分頁 1：股票追蹤 ---
    ws_stock = _get_worksheet(
        spreadsheet, STOCK_WORKSHEET,
        ["檢查日期", "股票代碼", "公司名稱", "PTT提及次數", "股價"],
    )
    # USER_ENTERED 讓 =GOOGLEFINANCE(...) 被當成公式而非文字
    ws_stock.append_rows(stock_rows, value_input_option="USER_ENTERED")
    print(f"[完成] {len(stock_rows)} 列已附加到「{SPREADSHEET_NAME}/{STOCK_WORKSHEET}」")

    # --- 分頁 2：熱門詞彙 ---
    ws_words = _get_worksheet(
        spreadsheet, WORDS_WORKSHEET,
        ["檢查日期", "排名", "詞彙", "出現次數"],
    )
    ws_words.append_rows(word_rows, value_input_option="USER_ENTERED")
    print(f"[完成] {len(word_rows)} 列已附加到「{SPREADSHEET_NAME}/{WORDS_WORKSHEET}」")
...
def main():
    ...
    write_to_google_sheets(stock_rows, word_rows)
